...
<div id="product-prices" class="panel product-tab">
	...
	<div class="form-group" {if !$country_display_tax_label || $tax_exclude_taxe_option}style="display:none;"{/if} >
		<label class="control-label col-lg-3 required" for="priceTI">{l s='Retail price with tax'}</label>
		<div class="input-group col-lg-2">
			<span class="input-group-addon">{$currency->prefix}{$currency->suffix}</span>
	...
	<div class="form-group">
		...
		<div class="col-lg-4">
			<div class="input-group">
				<!--<span class="input-group-addon">{$currency->prefix}{$currency->suffix}</span>-->
				<input type="hidden" id="unit_price" name="unit_price" type="text" value="{$unit_price|string_format:'%.6f'}" maxlength="27" onkeyup="if (isArrowKey(event)) return ;this.value = this.value.replace(/,/g, '.'); unitPriceWithTax('unit');"/>
